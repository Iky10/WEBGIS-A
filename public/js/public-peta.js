(function () {
    ...
    function makeIcon(kondisi) {
        ...
        var svg =
            ...
            '<path filter="url(#ds)" d="M15 2C8.37 2 3 7.37 3 14c0 8.9 12 26 12 26S27 22.9 27 14C27 7.37 21.63 2 15 2z" fill="' +
            c +
            '" stroke="rgba(255,255,255,.55)" stroke-width="1.5"/>' +
            '<circle cx="15" cy="14" r="5.5" fill="rgba(255,255,255,.92)"/>' +
            '<circle cx="15" cy="14" r="2.8" fill="' +
            c +
            '"/>' +
            "</svg>";
        return L.divIcon({
    ...
    function buildPopup(p, lat, lng) {
        ...
        return (
            '<div class="gis-popup" style="width:auto; padding:0;">' +
            '<div class="pu-body" style="padding:10px 14px;">' +
            '<div class="pu-name" style="font-size:0.95rem; margin:0;">' +
            p.nama_gedung +
            "</div>" +
            "</div></div>"
        );
    ...
    function renderMarkers(data) {
        ...
        var rpTotalEl = document.getElementById("rpTotal");
        if (rpTotalEl) rpTotalEl.textContent = data.length;
        updateLabels();
    }
    ...
    /* ── RIGHT PANEL FILTER ───────────────────── */
    var activeKondisi = new Set(); // kosong = semua tampil
    var activeFungsi = new Set(); // kosong = semua tampil

    function applyRightPanelFilter() {
        var filtered = allData.filter(function (f) {
            var p = f.properties;
            var kondisiVal = p.kondisi || "__unknown__";
            var fungsiVal = p.fungsi || "__unknown__";

            var kondisiOk =
                activeKondisi.size === 0 || activeKondisi.has(kondisiVal);
            var fungsiOk =
                activeFungsi.size === 0 || activeFungsi.has(fungsiVal);
            return kondisiOk && fungsiOk;
        });
        renderMarkers(filtered);
        updateRpCounts();
    }

    function updateRpCounts() {
        // Hitung per kondisi
        var counts = { Baik: 0, Sedang: 0, Rusak: 0, __unknown__: 0 };
        var fungsiCounts = {};

        allData.forEach(function (f) {
            var p = f.properties;
            var k = p.kondisi || "__unknown__";
            if (counts[k] !== undefined) counts[k]++;
            else counts["__unknown__"]++;

            var fn = p.fungsi || "__unknown__";
            fungsiCounts[fn] = (fungsiCounts[fn] || 0) + 1;
        });

        var el;
        if ((el = document.getElementById("cntBaik")))
            el.textContent = counts["Baik"];
        if ((el = document.getElementById("cntSedang")))
            el.textContent = counts["Sedang"];
        if ((el = document.getElementById("cntRusak")))
            el.textContent = counts["Rusak"];
        if ((el = document.getElementById("cntUnknown")))
            el.textContent = counts["__unknown__"];
        if ((el = document.getElementById("cntKondisiAll")))
            el.textContent = allData.length;
        if ((el = document.getElementById("cntFungsiAll")))
            el.textContent = allData.length;
        ...
        [
            ...
            var cnt = document.getElementById("cntFungsi" + fn);
            if (cnt) cnt.textContent = fungsiCounts[fn] || 0;
        });
    }

    // Checkbox logic — Kondisi
    document.querySelectorAll("#secKondisi .rp-item").forEach(function (item) {
        var cb = item.querySelector('input[type="checkbox"]');
        var val = item.dataset.kondisi;

        cb.addEventListener("change", function () {
            if (val === "") {
                // "Semua" → centang/uncentang semua
                var all = document.querySelectorAll(
                    '#secKondisi .rp-item:not([data-kondisi=""])',
                );
                all.forEach(function (i) {
                    i.querySelector("input").checked = cb.checked;
                });
                activeKondisi.clear();
                if (!cb.checked) {
                    // Tidak ada yang aktif → sembunyikan semua kondisi
                    ["Baik", "Sedang", "Rusak", "__unknown__"].forEach(
                        function (k) {
                            activeKondisi.add(k);
                        },
                    );
                    // Tapi kita ingin "none checked = semua tersembunyi"
                    // Ubah logika: kalau semua un-check, semua marker hilang
                    // Kalau "Semua" un-check, hapus semua kondisi dari active set tapi tandai "none"
                    activeKondisi = new Set(["__none__"]); // token khusus = kosongkan peta
                }
            } else {
                // Update "Semua" checkbox
                var allCb = document.querySelector(
                    '#secKondisi .rp-item[data-kondisi=""] input',
                );
                if (!cb.checked) {
                    activeKondisi.add(val);
                    if (allCb) allCb.checked = false;
                } else {
                    activeKondisi.delete(val);
                    // Cek apakah semua kondisi spesifik tercentang → centang "Semua"
                    ...
                    if (!anyUnchecked && allCb) allCb.checked = true;
                }
                // Bersihkan token __none__ jika ada
                activeKondisi.delete("__none__");
            }
            applyRightPanelFilter();
        });
    });

    // Checkbox logic — Fungsi
    document.querySelectorAll("#secFungsi .rp-item").forEach(function (item) {
        var cb = item.querySelector('input[type="checkbox"]');
        var val = item.dataset.fungsi;

        cb.addEventListener("change", function () {
            if (val === "") {
                var all = document.querySelectorAll(
                    '#secFungsi .rp-item:not([data-fungsi=""])',
                );
                all.forEach(function (i) {
                    i.querySelector("input").checked = cb.checked;
                });
                activeFungsi.clear();
                if (!cb.checked) {
                    activeFungsi = new Set(["__none__"]);
                }
            } else {
                var allCb = document.querySelector(
                    '#secFungsi .rp-item[data-fungsi=""] input',
                );
                if (!cb.checked) {
                    activeFungsi.add(val);
                    if (allCb) allCb.checked = false;
                } else {
                    activeFungsi.delete(val);
                    var anyUnchecked = false;
                    document
                        .querySelectorAll(
                            '#secFungsi .rp-item:not([data-fungsi=""])',
    ...
    document.querySelectorAll(".rp-sec-header").forEach(function (header) {
    ...
    document.getElementById("rpReset").addEventListener("click", function () {
        ...
        activeFungsi.clear();
        document
            .querySelectorAll('#rightPanel input[type="checkbox"]')
            .forEach(function (cb) {
                cb.checked = true;
            });
        renderMarkers(allData);
    ...
    // Toggle panel (sembunyikan/tampilkan)
    var rpPanel = document.getElementById("rightPanel");
    var rpToggle = document.getElementById("rpToggle");
    var rpOpenBtn;

    // Buat tombol buka panel
    rpOpenBtn = document.createElement("div");
    rpOpenBtn.id = "rpOpenBtn";
    rpOpenBtn.innerHTML = '<i class="fas fa-layer-group"></i>';
    rpOpenBtn.title = "Tampilkan Panel Filter";
    document.body.appendChild(rpOpenBtn);

    rpToggle.addEventListener("click", function () {
        rpPanel.classList.add("rp-collapsed");
        rpOpenBtn.classList.add("visible");
    });
    rpOpenBtn.addEventListener("click", function () {
        rpPanel.classList.remove("rp-collapsed");
        rpOpenBtn.classList.remove("visible");
    });

    // Update rpTotal di renderMarkers — tambahkan ini di akhir fungsi renderMarkers
    // (lihat Step 3c di bawah)
        ...
        sDrop.innerHTML = hits.length
            ? hits
                  .map(function (f, idx) {
                      ...
                      var isFirst =
                          idx === 0
                              ? ' style="background:rgba(59,130,246,.12); border-top:1px solid var(--accent-dim); border-bottom:1px solid var(--border);"'
                              : "";
                      return (
                          '<div class="t-drop-item" data-id="' +
                          p.id +
                          '" data-lat="' +
                          ...
                          '<div><div class="t-drop-name">' +
                          p.nama_gedung +
                          "</div>" +
                          '<div class="t-drop-sub">' +
                          (p.fungsi !== "-" ? p.fungsi + " · " : "") +
                          p.alamat +
                          "</div></div>" +
                          "</div>"
                  ...
                  .join("")
            : '<div class="t-drop-item"><div class="t-drop-sub" style="padding:4px 0; text-align:center;">Tidak ada hasil untuk "' +
              this.value +
              '"</div></div>';
    ...
    function showMarkerPreview(marker, lat, lng) {
        ...
        previewEl.innerHTML =
            ...
            data.nama +
            "</div>" +
            '<div class="mp-addr"><i class="fas fa-map-marker-alt"></i> ' +
            data.alamat +
            "</div>" +
            '<div class="mp-btns">' +
            '<button class="mp-btn detail" onclick="openSidebar(' +
            data.id +
            ')"><i class="fas fa-info-circle"></i> Detail</button>' +
            '<button class="mp-btn route" onclick="setRoutingDest(' +
            lat +
            "," +
            lng +
            ')"><i class="fas fa-directions"></i> Rute</button>' +
            "</div>" +
            "</div>";
    ...
    fetch(window.WEBGIS_URL)
        ...
        .then(function (gj) {
            ...
            if (allData.length) {
                var pts = allData.map(function (f) {
                    return [
                        f.geometry.coordinates[1],
                        f.geometry.coordinates[0],
                    ];
            ...
            if (uid) {
                var t = allData.find(function (f) {
                    return f.properties.id == uid;
                });
                if (t)
                    map.flyTo(
    ...
    function calculateRouteDirect(startLat, startLng, endLat, endLng, profile) {
        ...
        return fetch(url)
            .then(function (response) {
                ...
                if (!response.ok)
                    throw new Error("OSRM response not ok: " + response.status);
    ...
    function onRouteFound(e) {
        ...
        if (e.routes && e.routes[0]) {
            ...
            document.getElementById("routeInfoMode").textContent =
                modeLabel[routeProfile] || routeProfile;
        ...
        if (navigator.geolocation) {
            ...
            navigator.geolocation.getCurrentPosition(
                function (pos) {
                    ...
                    calculateRouteDirect(
                        ...
                        .catch(function (err) {
                            console.error(
                                "Failed to get initial route data:",
                                err,
                            );
                        });
                ...
                function () {
                    ...
                    toast(
                        "Gagal mendapatkan lokasi. Menggunakan titik tengah peta.",
                    );
                    ...
                    userMarker = L.circleMarker([centerLat, centerLng], {
                        ...
                        .addTo(map)
                        .bindPopup("📍 Lokasi Default (Pusat Peta)", {
                            closeButton: false,
                        });
                    ...
                    calculateRouteDirect(
                        ...
                        .catch(function (err) {
                            console.error(
                                "Failed to get fallback route data:",
                                err,
                            );
                        });
                },
            ...
            calculateRouteDirect(centerLat, centerLng, lat, lng, routeProfile)
                .then(function (routeData) {
                    ...
                    document.getElementById("routeInfoDuration").textContent =
                        duration;
                    document.getElementById("routeInfoDistance").textContent =
                        distance;
                    ...
                    document.getElementById("routeInfoMode").textContent =
                        modeLabel[routeProfile] || routeProfile;
                    document.getElementById("routeInfoPanel").style.display =
                        "block";
                    document.getElementById("btnResetRoute").style.display =
                        "flex";
                })
                .catch(function (err) {
    ...
    window.changeRouteMode = function (mode) {
        ...
        if (currentCoordinates.start && currentCoordinates.end) {
            console.log(
                "Changing mode with saved coordinates:",
                currentCoordinates,
            );
    ...
    document
        .getElementById("sbBtnPhotos")
        .addEventListener("click", function () {
            if (sbGallery.style.display === "none") {
                sbGallery.style.display = "block";
                this.innerHTML =
                    '<i class="fas fa-chevron-up"></i> Sembunyikan Foto';
            } else {
    ...
    window.openSidebar = function (id) {
        ...
        fetch("/api/gedung/" + id)
            ...
            .then(function (data) {
                ...
                document.getElementById("sbFungsi").textContent =
                    p.fungsi && p.fungsi !== "-" ? p.fungsi : "-";
                document.getElementById("sbKondisi").textContent =
                    p.kondisi && p.kondisi !== "-" ? p.kondisi : "-";
                document.getElementById("sbLantai").textContent =
                    p.jumlah_lantai && p.jumlah_lantai !== "-"
                        ? p.jumlah_lantai
                        : "-";
                document.getElementById("sbTahun").textContent =
                    p.tahun_berdiri && p.tahun_berdiri !== "-"
                        ? p.tahun_berdiri
                        : "-";

                // Deskripsi
                document.getElementById("sbDesc").innerHTML =
                    p.deskripsi || "-";

                // Photos
                var grid = document.getElementById("sbGalleryGrid");
                if (data.fotos && data.fotos.length > 0) {
                    document.getElementById("sbBtnPhotos").style.display =
                        "flex";
                    grid.innerHTML = data.fotos
                        .map(function (f) {
                            return (
                                '<a href="' +
                                f.path +
                    ...
                    document.getElementById("sbBtnPhotos").style.display =
                        "none";
                    grid.innerHTML =
                        '<div style="grid-column:1/3; color:var(--muted); font-size:0.8rem; text-align:center;">Belum ada foto galeri</div>';
                }
                ...
                if (data.fasilitas && data.fasilitas.length > 0) {
                    ...
                    data.fasilitas.forEach(function (f) {
                        var statusColor = f.is_aktif
                            ? "var(--success)"
                            : "var(--muted)";
                        var statusText = f.is_aktif
                            ? "Sedang Dipakai"
                            : "Kosong";
                        var pulseClass = f.is_aktif ? "pulse-mini" : "";

                        fasHtml +=
                            '<div style="background:rgba(255,255,255,0.03); border:1px solid var(--border); border-radius:12px; padding:12px; transition:all 0.3s ease;">' +
                            '<div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:4px;">' +
                            '<strong style="font-size:0.9rem; color:var(--text);">' +
                            f.nama_fasilitas +
                            "</strong>" +
                            (f.kategori
                                ? '<span style="font-size:0.65rem; color:var(--accent); font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border:1px solid var(--accent-dim); padding:2px 8px; border-radius:100px; background:var(--accent-dim);">' +
                                  f.kategori +
                                  "</span>"
                                : "") +
                            "</div>" +
                            ...
                            (f.is_aktif
                                ...
                                : "transparent") +
                            '"></div>' +
                            '<span style="font-size:0.7rem; font-weight:600; color:' +
                            statusColor +
                    ...
                    fasEl.innerHTML =
                        '<div style="background:rgba(255,255,255,0.03); border:1px dashed var(--border); border-radius:12px; padding:16px; text-align:center; color:var(--muted); font-size:0.8rem;">Informasi fasilitas & kelas pada gedung ini belum tersedia saat ini.</div>';
                }
            })
            .catch(function (err) {
                console.error(err);
